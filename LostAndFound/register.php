...
if(isset($_POST['register'])) {
    ...
    $message = "Error: All fields are required!";
        ...
        if(mysqli_num_rows($email_result) > 0) {
            ...
            $sql = "INSERT INTO accounts (role, name, contactnum, email, password, is_2fa_enabled)
                    VALUES ('$role', '$name', '$contactnum', '$email', '$hashed_password', 1)";
                    if(mysqli_query($connect, $sql)) {
    // Get ID new user registration 
    $new_user_id = mysqli_insert_id($connect);
    
    // Send OTP for verification
    $twoFA = new Register2FA($connect);
    $otp_result = $twoFA->sendVerificationOTP($new_user_id, $email, $name);
    
    if($otp_result['success']) {
        // Store user_id in session for OTP process
        $_SESSION['pending_user_id'] = $new_user_id;
        $_SESSION['pending_user_email'] = $email;
        
        // Redirect to verify OTP page
        $_SESSION['just_registered'] = true;
        header("Location: verify-register-otp.php");
        exit();
    } else {
        $message = "Registration successful tapi gagal hantar OTP: " . $otp_result['message'];
    }
    
    // ===== NOTIFICATION FOR ADMIN =====
    $notif_title = "New User Registration";
    $notif_message = "New user '" . addslashes($name) . "' has registered.";
    $notif_link = "admin_users.php";
    ...
    while($admin = mysqli_fetch_assoc($admin_result)) {
        ...
        $notif_sql = "INSERT INTO notifications (user_id, type, title, message, link) 
                      VALUES ('$admin_id', 'new_user', '$title_esc', '$msg_esc', '$link_esc')";
